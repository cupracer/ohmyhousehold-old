...
class SupplyVoter extends Voter
{
    ...
    protected function supports(string $attribute, $subject): bool
    {
        ...
        // only vote on `Supply` objects
        if (!$subject instanceof Supply) {
            return false;
        }
    ...
    protected function voteOnAttribute(string $attribute, $subject, TokenInterface $token): bool
    {
        ...
        // you know $subject is an Supply object, thanks to `supports()`
        /** @var Supply $supply */
        $supply = $subject;

        $householdUser = $this->householdUserRepository->findOneBy([
            'user' => $user,
            'household' => $supply->getHousehold()
        ]);
    ...
    {
        // no further permissions than 'view' required to edit supplies
        return $this->canView($householdUser);
    }
