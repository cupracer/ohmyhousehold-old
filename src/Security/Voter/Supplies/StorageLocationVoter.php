...
class StorageLocationVoter extends Voter
{
    ...
    protected function supports(string $attribute, $subject): bool
    {
        ...
        // only vote on `Storagelocation` objects
        if (!$subject instanceof Storagelocation) {
            return false;
        }
    ...
    protected function voteOnAttribute(string $attribute, $subject, TokenInterface $token): bool
    {
        ...
        // you know $subject is an Storagelocation object, thanks to `supports()`
        /** @var Storagelocation $storagelocation */
        $storagelocation = $subject;

        $householdUser = $this->householdUserRepository->findOneBy([
            'user' => $user,
            'household' => $storagelocation->getHousehold()
        ]);
    ...
    {
        // no further permissions than 'view' required to edit storagelocations
        return $this->canView($householdUser);
    }
