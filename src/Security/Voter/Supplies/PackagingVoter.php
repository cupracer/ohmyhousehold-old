...
class PackagingVoter extends Voter
{
    ...
    protected function supports(string $attribute, $subject): bool
    {
        ...
        // only vote on `Packaging` objects
        if (!$subject instanceof Packaging) {
            return false;
        }
    ...
    protected function voteOnAttribute(string $attribute, $subject, TokenInterface $token): bool
    {
        ...
        // you know $subject is an Packaging object, thanks to `supports()`
        /** @var Packaging $packaging */
        $packaging = $subject;

        $householdUser = $this->householdUserRepository->findOneBy([
            'user' => $user,
            'household' => $packaging->getHousehold()
        ]);
    ...
    {
        // no further permissions than 'view' required to edit packagings
        return $this->canView($householdUser);
    }
